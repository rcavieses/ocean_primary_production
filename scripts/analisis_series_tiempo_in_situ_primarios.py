...
OUTPUT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + '/resultados_series_tiempo'
...
df = pd.read_csv(os.path.join(DATA_DIR, 'taxonomy_corrected_edna.csv'))

print(f"Registros totales: {len(df)}")

# Convertir fecha de Excel al formato datetime
# La columna se llama 'Collected date year' pero contiene números seriales de Excel
def excel_date_to_datetime(excel_date):
    ...
    try:
        if isinstance(excel_date, str):
            # Si es texto como "23/06/2018", parsear directamente
            if '/' in excel_date:
                return pd.to_datetime(excel_date, format='%d/%m/%Y', errors='coerce')
            else:
                excel_date = float(excel_date)
        # Convertir número serial de Excel
        # Excel inicia en 1/1/1900 = serial 1
